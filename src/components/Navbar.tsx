...
const Navbar: FC = () => {
  ...
  return (
    <>
      <motion.div
        className="flex justify-between items-center mt-3 lg:w-[90vw] mx-auto"
        transition={{ duration: 1.5, delay: 0.4, opacity: { ease: "linear" } }}
        initial={{ y: "-10vw", opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
      >
        <Link href="/">
          <span className="ml-4 md:translate-x-[-1.4rem]">
            <Image
              src="/logo.svg"
              alt="logo"
              width="50"
              height="50"
              className="rotate-[-8deg]  md:translate-y-3"
            />
          </span>
        </Link>
        ...
        <div
          className={`${dropDown}  fixed inset-y-[10vh]   bg-[#210d2c]  w-[100vw] h-[100vh] lg:bg-transparent md:static md:flex md:gap-12 md:w-fit md:h-fit md:bg-transparent lg:gap-20 `}
        >
          {navItems.map((item, index) => {
            if (item) {
              return (
                <NavItem key={index} linkTo={item}>
                  {item}
                </NavItem>
              );
            }
